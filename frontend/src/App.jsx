...
import { buildOverview, buildPoolSummary, parseUigfAccounts } from "./uigf";
import { enrichRecords, loadItemLookup, resolveItemId } from "./itemLookup";
import {
  loadLocalState,
  saveAccountResult,
  setDefaultUid,
...
const poolsWithoutUpPityStats = new Set(["100", "200", "500"]);
...
function getIconUrl(row, lookup) {
  const itemId =
    `${row.itemId || ""}`.trim() ||
    resolveItemId(
      { name: row.name, item_type: row.itemType, item_id: row.itemId },
      lookup,
    );
  if (!itemId) return "";
  const itemType = `${row.itemType || ""}`.trim();
  if (itemType === "角色") {
    return `http://127.0.0.1:8001/icon/character/${itemId}.png`;
  }
  if (itemType === "武器") {
    return `http://127.0.0.1:8001/icon/weapon/${itemId}.png`;
  }
  return "";
}
...
function FourStarGapRow({ items, lookup, flush = false }) {
      ...
      {items.map((item) => (
        <div className="four-star-gap-item" key={item.key || item.id}>
          <FiveStarIconWithPlaceholder
            iconUrl={getIconUrl(item, lookup)}
            name={item.name}
            rankType="4"
          />
          {item.count > 1 ? (
...
function showNonUpBadge(gachaType, isUp) {
  if (`${gachaType}` === "500") return false;
  return isUp === false;
}
...
export default function App() {
  ...
  useEffect(() => {
    let cancelled = false;
    loadItemLookup().then((lookup) => {
      if (!cancelled) setItemLookup(lookup);
    });
    return () => {
      cancelled = true;
    };
  ...
  const applyPoolOrderAfterDrag = (fromGacha, toGacha) => {
    ...
    setFiveStarDisplay((prev) => {
      const merged = mergeOrderWithData(prev.order, poolTypesFromData);
      const from = merged.indexOf(fromGacha);
      const to = merged.indexOf(toGacha);
      if (from < 0 || to < 0 || from === to) return prev;
      const nextOrder = merged.slice();
      const [moved] = nextOrder.splice(from, 1);
      nextOrder.splice(to, 0, moved);
      const next = {
        ...prev,
        order: mergeOrderWithRest(nextOrder, prev.order),
      };
      saveFiveStarDisplaySettings(next);
      return next;
    });
  ...
  const applyResult = async (data, source) => {
    const lookup = itemLookup || (await loadItemLookup());
    if (lookup && !itemLookup) setItemLookup(lookup);
    const withHistory = {
      ...data,
      records: enrichRecords(
        annotateUpByHistory(data?.records || [], normalizedHistory),
        lookup,
      ),
    };
    ...
    (async () => {
      ...
      for (const [uid, account] of Object.entries(state.accounts || {})) {
        ...
        const records = enrichRecords(
          annotateUpByHistory(baseResult.records, normalizedHistory),
          itemLookup,
        );
        nextAccounts[uid] = {
          ...account,
  ...
  const onAutoFetch = async () => {
    try {
      ...
      if (data.wish_url) {
        setWishUrl(data.wish_url);
        setLogs((prev) => [...prev, `[自动提取] 抽卡分析链接: ${data.wish_url}`]);
      }
      appendLog("自动流程完成");
    } catch (err) {
  ...
  const onImportUigf = async (event) => {
    ...
    try {
      ...
      appendLog(`解析到 ${accounts.length} 个账号`);
      ...
      appendLog(
        accounts.length > 1
          ? `本地 UIGF 解析完成，已导入 ${accounts.length} 个账号`
          : "本地 UIGF 解析完成",
      );
    } catch (err) {
    ...
    <main className="page">
      ...
      <section className="panel">
        ...
        <div className="action-buttons">
          ...
          {wishUrl ? <span className="current-url">当前URL已缓存</span> : null}
        </div>
      ...
      {showUrlDialog ? (
        <div className="dialog-mask">
          <div className="dialog">
            <h3>输入祈愿历史链接</h3>
            <textarea
              placeholder="粘贴游戏祈愿历史链接（含 authkey）"
              value={urlDraft}
            ...
            {displayPoolOrder.length ? (
              <div className="five-star-groups">
                {displayPoolOrder.map((gachaType) => {
                  ...
                  const gt = `${gachaType}`;
                  const pityLimit = getPoolPityLimit(gt);
                  ...
                  const pityPercent = Math.min(100, (sinceLastFive / pityLimit) * 100);
                  const pityClass = getPityClass(sinceLastFive);

                  return (
                    <div className="five-star-group" key={gachaType}>
                      <h3>{typeNameMap[gt] || gt}</h3>
                      <div className="five-star-row five-star-row-pending">
                        <div className="five-star-meta five-star-meta--pending">
                          <span className="five-star-time">距上一五星已 {sinceLastFive} 抽</span>
                        </div>
                        <div className="five-star-bar-wrap">
                          <div className="five-star-track">
                            <div
                              className={`five-star-bar ${pityClass}`}
                              style={{ width: `${pityPercent}%` }}
                            />
                          </div>
                          <span className="five-star-count">{sinceLastFive} / {pityLimit}</span>
                        </div>
                        {fiveStarDisplay.showFourStar ? (
                          <FourStarGapRow items={pendingFourStars} lookup={itemLookup} flush />
                        ) : null}
                      </div>
                      {poolRows.length ? (
                        <div className="five-star-list">
                          {poolRows.map((row) => {
                            const rowPityPercent = Math.min(100, (row.pity / pityLimit) * 100);
                            const rowPityClass = getPityClass(row.pity);
                            const iconUrl = getIconUrl(row, itemLookup);
                            return (
                              <div className="five-star-row" key={row.id}>
                                ...
                                <div className="five-star-bar-wrap">
                                  ...
                                  <div className="five-star-track">
                                    <div
                                      className={`five-star-bar ${rowPityClass}`}
                                      style={{ width: `${rowPityPercent}%` }}
                                    />
                                  </div>
                                  {showNonUpBadge(gt, row.isUp) ? (
                                    <span className="non-up-badge">歪</span>
                                  ) : null}
                                  <span className="five-star-count">{row.pity} 抽</span>
                                </div>
                                {fiveStarDisplay.showFourStar ? (
                                  <FourStarGapRow items={row.fourStars} lookup={itemLookup} />
                                ) : null}
                              </div>
                            );
                          })}
                        ...
                        <div className="log-empty">暂无五星记录。</div>
                      )}
                    </div>
                  );
                })}
              ...
              <div className="log-empty">暂无祈愿记录。</div>
            )}
          </section>
          ...
          <section className="panel">
            ...
            <div className="records">
              {recentRecords.map((item) => {
                const rank = `${item.rank_type || ""}`;
                const rankClass = rank === "5" ? "rank-5" : rank === "4" ? "rank-4" : "";
                return (
                  <div className={`record ${rankClass}`} key={item.id}>
                    <div className="record-main">
                      <span>
                        <strong>{item.name}</strong>
                        <span className="record-type-badge">
                          {typeNameMap[item.gacha_type] || item.gacha_type}
                        </span>
                      </span>
                      <span>{item.time}</span>
                    </div>
      ...
      {showFiveStarSettings ? (
        <div className="dialog-mask" onClick={closeFiveStarSettings}>
          <div
            className="dialog five-star-settings-dialog"
          ...
          >
            ...
            <div className="five-star-settings-list">
              {mergeOrderWithData(fiveStarDisplay.order, poolTypesFromData).map((gachaType) => {
                const isVisible = fiveStarDisplay.visible[gachaType] !== false;
                const isDragSource = poolDragGacha === gachaType;
                const isDragOver =
                  poolDragOverGacha === gachaType && poolDragGacha && poolDragGacha !== gachaType;
                return (
                  <div
                    key={gachaType}
                    className={`five-star-settings-row ${isDragSource ? "five-star-settings-row--source" : ""} ${isDragOver ? "five-star-settings-row--over" : ""}`}
                    onDragOver={(e) => {
                      e.preventDefault();
                  ...
                  >
                    ...
                    <label className="five-star-settings-check">
                      ...
                      />
                      <span>{typeNameMap[gachaType] || gachaType}</span>
                    </label>
                  </div>
                );
              })}
            ...
            <div className="dialog-actions">
              <button type="button" className="small-btn gray" onClick={closeFiveStarSettings}>
                关闭
              </button>
            </div>
          </div>
