...
impl LauncherApp {
    ...
    fn ensure_server_thread(&mut self) {
        ...
        self.server_thread = Some(std::thread::spawn(move || {
            ...
            if let Err(e) = rt.block_on(genshin_gacha_api::run_server(&addr)) {
        ...
        egui::CentralPanel::default().show(ctx, |ui| {
            ui.vertical_centered(|ui| {
                ...
                if open_clicked {
                    ...
                    self.status = "等待 127.0.0.1:8001 就绪…".to_string();
                    if Self::wait_for_port("127.0.0.1", 8001, 100, Duration::from_millis(100)) {
                        ...
                        self.status =
                            "超时：8001 端口未监听。若端口被占用，请关闭占用程序后重试。"
                                .to_string();
                    }
                }
...
fn launcher_log_path() -> PathBuf {
    std::env::current_exe()
        .ok()
        .and_then(|p| p.parent().map(|d| d.join("genshin-gacha-launcher.log")))
        .unwrap_or_else(|| PathBuf::from("genshin-gacha-launcher.log"))
}

fn main() -> eframe::Result<()> {
    let log_path = launcher_log_path();
    match genshin_gacha_api::init_tracing_to_file(&log_path) {
