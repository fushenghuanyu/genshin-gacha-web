...
const DEFAULT_TYPES: [&str; 5] = ["100", "200", "301", "302", "500"];
...
const GACHA_LOG_PATH: &str = "/gacha_info/api/getGachaLog";

pub fn query_from_wish_url(wish_url: &str) -> Result<(String, IndexMap<String, String>), String> {
    let parsed = Url::parse(wish_url).map_err(|e| e.to_string())?;
    ...
    if auth.is_empty() {
        return Err("链接中缺少 authkey，请重新从游戏祈愿历史页面复制。".to_string());
    }
    ...
    F: FnMut(String),
{
    for attempt in 0..4 {
        ...
        if message == "authkey timeout" {
            return Err("authkey 已过期，请在游戏内重新打开祈愿历史后再复制链接。".to_string());
        }
...
pub async fn resolve_uid_from_wish_url(wish_url: &str) -> Result<Option<String>, String> {
    let (host, query) = query_from_wish_url(wish_url)?;
    let api_domain = detect_api_domain(&host);
    let base_url = format!("{api_domain}{GACHA_LOG_PATH}");
    let client = Client::new();

    let mut params = query.clone();
    params.insert("gacha_type".into(), "100".into());
    params.insert("page".into(), "1".into());
    params.insert("size".into(), "5".into());
    params.insert("end_id".into(), "0".into());

    let mut noop = |_msg: String| {};
    let payload = request_with_retry(&client, &base_url, &params, "100", &mut noop).await?;
    let list = payload
        .get("data")
