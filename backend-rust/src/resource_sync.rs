...
const DEFAULT_GITEE_REPO: &str = "genshin-gacha-web";
const DEFAULT_GITEE_BRANCH: &str = "main";
const DEFAULT_GITHUB_OWNER: &str = "fushenghuanyu";
const DEFAULT_GITHUB_REPO: &str = "genshin-gacha-web";
...
fn http_client() -> Result<Client, SyncError> {
    ...
    headers.insert(
        USER_AGENT,
        HeaderValue::from_str(&format!("genshin-gacha-launcher/{}", crate::APP_VERSION))
...
mod tests {
    ...
    fn local_match_uses_blob_sha() {
        let dir = std::env::temp_dir().join(format!("gi-sync-test-{}", std::process::id()));
    ...
    fn gitee_source_urls() {
        let source = RemoteRepoSource {
            ...
            repo: "genshin-gacha-web".into(),
            branch: "main".into(),
        };
        assert_eq!(
            source.raw_url("resources/gacha/character.json"),
            "https://gitee.com/floating-illusion-language/genshin-gacha-web/raw/main/resources/gacha/character.json"
        );
