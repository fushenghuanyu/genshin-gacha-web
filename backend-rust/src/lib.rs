...
use error::AppError;
use genshin_client::{fetch_all_records, resolve_uid_from_wish_url};
...
pub async fn run_server(addr: &str) -> io::Result<()> {
    ...
    tracing::info!("原神 Gacha API (Rust) 监听于 http://{addr}");
...
fn app_router(state: AppState) -> Router {
    ...
    let mut r = Router::new()
        ...
        .route("/api/item-lookup", get(item_lookup_api));
...
async fn history(State(st): State<Arc<AppState>>) -> Result<axum::Json<Value>, AppError> {
    let rows = gi_history::load_history_rows(&st.project_root).map_err(|e| {
        if e.contains("未找到") {
            AppError::NotFound {
                message: e,
                logs: vec![],
...
async fn item_lookup_api(State(st): State<Arc<AppState>>) -> axum::Json<Value> {
    let lookup = item_lookup::ItemLookup::global(&st.project_root);
    axum::Json(lookup.as_json())
}

async fn user_data_bootstrap(State(st): State<Arc<AppState>>) -> Result<axum::Json<Value>, AppError> {
    let mut v = user_data::load_bootstrap(&st.user_data_dir).map_err(|e| AppError::Server {
        message: e,
        logs: vec![],
    })?;
    item_lookup::ItemLookup::global(&st.project_root).enrich_bootstrap(&mut v);
    Ok(axum::Json(v))
}

async fn user_data_save(
    State(st): State<Arc<AppState>>,
    axum::Json(mut body): axum::Json<user_data::PersistBody>,
) -> Result<axum::Json<Value>, AppError> {
    item_lookup::ItemLookup::global(&st.project_root).enrich_persist_body(&mut body);
...
fn records_to_response_value(
    ...
    item_lookup::ItemLookup::global(project_root).enrich_records(&mut records);
...
async fn fetch_gacha_core(
    ...
    let mut client_lines: Vec<String> = Vec::new();
    let local_latest_ids = if fetch_full_history {
        None
    } else {
        match resolve_uid_from_wish_url(&wish_url).await {
    ...
    records_to_response_value(project_root, wish_url, fetched.records, logs)
}
...
async fn fetch_gacha(
    ...
    let wish_url = body.wish_url.trim().to_string();
    if wish_url.len() < 10 {
        return Err(AppError::BadRequest {
            message: "wish_url 过短".into(),
            logs: vec![],
        });
    }
...
async fn fetch_gacha_auto(
    ...
    let wish = match auto_url::get_wish_url_from_local_logs(|m| {
...
fn ndjson_fetch_stream(
    ...
    async_stream::stream! {
        ...
        let w = wish_url.clone();
        let ud = user_data_dir.clone();
        let handle = tokio::spawn(async move {
            let local_latest_ids = if fetch_full_history {
                None
            } else {
                match resolve_uid_from_wish_url(&w).await {
        ...
        match fetch_result {
            Ok(records) => match records_to_response_value(&project_root, wish_url, records, vec![]) {
                Ok(data) => {
...
fn ndjson_stream_response(
    ...
    let stream = ndjson_fetch_stream(
        project_root,
        user_data_dir,
        wish_url,
        fetch_full_history,
